...
public class MutualFundPage extends CommonMutualFundPage {
    ...
    public boolean selectCategory(String category) {
        ...
        for (int i = 0; i < MFWidgetSliderCategories.size(); i++) {
           ...
           categoryFound= category.equalsIgnoreCase(commonFunctions.getElementText(MFWidgetSliderCategories.get(i), 10));
            if (categoryFound) {
        ...
        if(!categoryFound){
                Utils.logStepInfo(false, "Expected category  "+category+" not found in widget");
        }
